...
func TestLoginCredentials_UnknownEndpointWithExplicitRegion(t *testing.T) {
	creds, err := loginCredentials(auth.AuthTypePAT, "https://api.self-hosted.example", selfHostedConfig(), "EU", true)
	if err != nil {
	...
	if creds.AuthEndpoint != "https://auth.self-hosted.example" ||
		creds.APIURL != "https://api.self-hosted.example" ||
		creds.ClientID != "self-hosted-client-id" ||
		creds.APIResource != "https://api.self-hosted.example" {
		t.Fatalf("the self-hosted endpoints used at login must be persisted, got %+v", creds)
	}
}
...
// Non-regression for root cause B: a self-hosted login must keep the following
// commands on self-hosted instead of silently retargeting prod US.
func TestSelfHostedLoginKeepsFollowingCommandsOnSelfHosted(t *testing.T) {
	creds, err := loginCredentials(auth.AuthTypePAT, "https://api.self-hosted.example", selfHostedConfig(), "us", true)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	creds.PAT = "pat_self_hosted"
	...
	if res.APIURL != "https://api.self-hosted.example" {
		t.Fatalf("expected the next command to stay on self-hosted, got %q", res.APIURL)
	}
	if res.AuthEndpoint != "https://auth.self-hosted.example" {
