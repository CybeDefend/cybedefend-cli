...
func TestResolveAuth_StoredEndpointsAreUsedVerbatim(t *testing.T) {
	...
	if res.APIURL != "https://api-staging.cybedefend.com" ||
		res.AuthEndpoint != "https://auth-staging.cybedefend.com" ||
		res.ClientID != "or88kgjv12sp7te58anpk" ||
		res.APIResource != "https://api-staging.cybedefend.com" {
		t.Fatalf("expected the persisted staging endpoints, got %+v", res)
	}
}
