...
func TestExchangeToken_UsesTheClientEndpoints(t *testing.T) {
	...
	client := NewClient("https://api-staging.cybedefend.com", "pat_staging", srv.URL,
		"or88kgjv12sp7te58anpk", "https://api-staging.cybedefend.com")
	...
	if got.Get("client_id") != "or88kgjv12sp7te58anpk" {
		t.Errorf("unexpected client_id %q", got.Get("client_id"))
	}
	if got.Get("resource") != "https://api-staging.cybedefend.com" {
		t.Errorf("unexpected resource %q", got.Get("resource"))
	}
	if got.Get("subject_token") != "pat_staging" {
...
// on the region and silently jumps to another instance (DEV-66, root cause B).
