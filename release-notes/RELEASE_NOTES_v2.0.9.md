...
Version v2.0.9 — September 8, 2026

- Built for multiple platforms
- Precompiled binaries included
- Docker image available at: `ghcr.io/CybeDefend/cybedefend-cli:v2.0.9`

Two fixes that matter for CI: `--break-on-severity` was never blocking a build, and every `report` command was writing an empty file while reporting success. Both affect v2.0.8 and earlier. Upgrade if you gate a pipeline on this CLI.
...
- A `report` command against an unreachable or failing report now exits non-zero instead of writing an empty file.
